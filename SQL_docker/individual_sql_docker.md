...
1. First be sure that you've installed and tested Docker and PostgreSQL as described by the "Install_Docker_and_Postgres_Guide.md" slacked to you in the `SQL_Docker.zip` compressed folder.

1. Start your postgres container, if it isn't already running.
   ```bash
   $ docker start postgres_container
   ```

1. Get inside the postgres container using bash.
   ```
   $ docker exec -it postgres_container bash
   ```

1. Create an empty `readychef` database.

   ```bash
   # createdb -U postgres readychef
   ```

1. Import data into it the `readychef` database.
   ```bash
   # psql -U postgres readychef < ./data/readychef.sql
   ```
   You should see a bunch of SQL commands flow through the terminal. 

1. Connect to the database using the `psql` interactive shell.
   ```bash
   # psql -U postgres -d readychef
   ```
